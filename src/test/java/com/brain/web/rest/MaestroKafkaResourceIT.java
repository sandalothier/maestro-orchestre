...
import com.brain.config.KafkaProperties;
...
class MaestroKafkaResourceIT {
    ...
    void setup() {
        ...
        MaestroKafkaResource kafkaResource = new MaestroKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/maestro-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/maestro-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
